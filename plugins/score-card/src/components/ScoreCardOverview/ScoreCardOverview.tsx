...
export const ScoreCardOverview = ({
  ...
  return (
    <InfoCard
      ...
      variant={variant}
      deepLink={{ title: 'Go to score details', link: '#' }}
      headerProps={{
        action: qualityBadge,
        classes: {
          root: classes.header,
      ...
      {!loading && data && (
        <div data-testid="score-card">
          <Grid
            container
            >
            {data.areaScores.map(areaScore => (
              <Grid item>
                <GaugeCard title={areaScore.title} key={areaScore.title} progress={areaScore.scorePercent/100} />
              </Grid>
            ))}
            </Grid>
        </div>
